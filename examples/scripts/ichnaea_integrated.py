"""Artifact-first variant of the ichnaea compact compute example."""
...
from sophios.compute_submit import submit_compute_json
...
def create_compute_json(workflow_id: str, cwl_workflow: Json, cwl_job_inputs: Json) -> Json:
    """Returns a compute compatible Json"""
    ...
    # =========== GET RAW SUBMITABLE COMPUTE JSON ====
    compute_json = compute_object.get_compute_payload()
    return compute_json
...
def main() -> int:
    ...
    parser.add_argument(
        "--submit-url",
        help="Optional compute create-workflow endpoint. If omitted, only write artifacts to disk.",
    )
    ...
    # construct compute json
    compute_json = create_compute_json(
        workflow_name, compiled_cwl_workflow, workflow_inputs)
    # write compute json before submit to disk for verifiability (optional)
    ...
    # dynamically alter compute job id before submit. so that we can distinguish between different jobs
    compute_json['id'] = compute_json['id'] + '__' + \
    ...
    if args.submit_url is None:
        print(
            ...
            f"and compute_{workflow_name}_integrated.json. Pass --submit-url to submit the JSON payload."
        )
        return 0

    # =========  SUBMIT TO COMPUTE ===================
    submission_status: int = submit_compute_json(compute_json, args.submit_url)
