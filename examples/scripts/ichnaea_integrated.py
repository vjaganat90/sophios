...
def workflow(input_dicts: Dict[str, str], workflow_name: str) -> Workflow:
    ...
    autoseg_clt.save(Path(__file__).with_name('built-ichnaea-autosegmentation.cwl'), validate=True)
    # =========== CREATE A STEP ======================
...
def main() -> int:
    ...
    # construct compute json
    compute_json = create_compute_json(workflow_name, compiled_cwl_workflow, workflow_inputs)
    ...
    compute_json['id'] = compute_json['id'] + '__' + datetime.now().strftime('%Y_%m_%d_%H.%M.%S') + '__'

    # =========  SUBMIT TO COMPUTE ===================
    return submit_compute_json(compute_json, BASE_URL)
