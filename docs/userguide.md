...
Intermediate compiler `.wic` trees are not written under `autogenerated/`, and
there is no flag that writes them. The compiler no longer assembles a document
of its own to write: it compiles the files you wrote, so the intermediate tree
those trees used to show does not exist.
