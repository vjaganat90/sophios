...
def main() -> None:
    ...
    if args.target == 'nextflow' and (args.run_local or args.generate_run_script):
        cli.parser.error(
            '--target nextflow cannot be combined with --run_local or --generate_run_script; '
            'generate the artifacts and execute workflow.nf with Nextflow'
        )
    ...
    if args.target == 'nextflow':
        if not args.generate_cwl_workflow:
            print('Please specify --generate_cwl_workflow when using --target nextflow')
            sys.exit(1)
        io_nf.write_nextflow_artifacts(
            cwl_rosetree_to_nextflow(rose_tree),
            Path('autogenerated/'),
        )
