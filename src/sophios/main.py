...
from .utils_nf import compiled_source_to_nextflow
...
def _load_source_bundle(args: argparse.Namespace,
    ...
    bundle = frontdoor.bundle_from_disk(
        Path(yaml_path), yml_paths, tools_cwl,
        None if args.ignore_validation_errors else validator)
...
def _write_nextflow_target(
    compilation: CompilationResult,
    yaml_stem: str,
) -> tuple[Path, Path, Path, Path]:
    """Lower the completed semantic graph and write Nextflow artifacts."""
    try:
        workflow = compiled_source_to_nextflow(compilation)
...
def _main() -> None:
    ...
    yaml_path, yaml_stem, bundle = _load_source_bundle(args, yml_paths, tools_cwl, validator)
    ...
    if args.target == 'nextflow':
        _write_nextflow_target(compilation, yaml_stem)
        return
