...
from . import input_output_nf as io_nf
...
from .utils_nf import cwl_rosetree_to_nextflow


def _write_intermediate_wic(yaml_stem: str, suffix: str, yaml_doc: Yaml, *, enabled: bool) -> Path | None:
    """Write an intermediate compiler `.wic` tree only when explicitly enabled."""
    if not enabled:
        return None
    output_path = Path('autogenerated') / f'{yaml_stem}_{suffix}.wic'
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(yaml.dump(yaml_doc), encoding='utf-8')
    return output_path
...
def _load_and_prepare_yaml_tree(args: argparse.Namespace, yml_paths: dict[str, dict[str, Path]],
    ...
    wic = {'wic': root_yaml_tree.get('wic') or {}}
    ...
    # Optionally write compiler-internal workflow trees for debugging.
...
def _write_nextflow_target(rose_tree: RoseTree, yaml_stem: str) -> tuple[Path, Path, Path, Path]:
    """Convert once, write Nextflow artifacts, and present conversion failures cleanly."""
    try:
        workflow = cwl_rosetree_to_nextflow(rose_tree)
        return io_nf.write_nextflow_artifacts(workflow, Path('autogenerated/'))
    except ValueError as error:
        message = str(error)
        if 'nested workflows' in message:
            message += '; retry with --cwl_inline_subworkflows when flattening is valid'
        error_path = Path(f'error_{yaml_stem}.txt')
        error_path.write_text(f'{message}\n', encoding='utf-8')
        print(f'Failed to generate Nextflow artifacts. See {error_path}.')
...
def main() -> None:
    ...
    if args.target == 'nextflow':
        _write_nextflow_target(rose_tree, yaml_stem)
    elif args.run_local or args.generate_run_script:
