...
class Workflow(BaseModel):
    ...
    def run(self, compile_args_dict: Dict[str, str] = {}, run_args_dict: Dict[str, str] = {}) -> None:
        ...
        input_output.write_to_disk(rose_tree, Path(
            'autogenerated/'), True, compile_args_dict.get('inputs_file', ''))
        # prepare for running
        post_compile.cwl_docker_extract(compile_args_dict.get(
            'container_engine', 'docker'), compile_args_dict.get('pull_dir', str(Path.cwd())),
            self.process_name)
        if compile_args_dict.get('docker_remove_entrypoints', None):
            rose_tree = post_compile.remove_entrypoints(
                compile_args_dict.get('container_engine', 'docker'), rose_tree)
        user_args = self._convert_args_dict_to_args_list(
            {**compile_args_dict, **run_args_dict})
        ...
        if args.toil_passthrough_flags == 'yes':
            ...
            run_local_module.run_local(
                args, rose_tree, args.cachedir, args.cwl_runner, True)
