...
class Workflow(BaseModel):
    ...
    def run(self, run_args_dict: Dict[str, str] = {}) -> None:
        ...
        input_output.write_to_disk(rose_tree, Path(
            'autogenerated/'), True, run_args_dict.get('inputs_file', ''))
        # prepare for running
        post_compile.cwl_docker_extract(run_args_dict.get(
        ...
        if run_args_dict.get('docker_remove_entrypoints', None):
            rose_tree = post_compile.remove_entrypoints(
                run_args_dict.get('container_engine', 'docker'), rose_tree)
