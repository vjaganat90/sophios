...
def compiled_workflow_from_compiler_info(
    ...
    return CompiledWorkflow(
        ...
        # Empty, not the newest known version, when the artifact carries no
        # annotation: reporting `LANG_VERSION` there would state a version the
        # document never claimed, which is the silent selection the resolver
        # exists to prevent. Matches `CompiledWorkflow.lang_version`'s own
        # default, so "unannotated" reads as unannotated. Unreachable while
        # the compiler always annotates — which is exactly when a wrong answer
        # would go unnoticed.
        lang_version=str(cwl_workflow.get(versions.ANNOTATION_KEY, '')),
    )
