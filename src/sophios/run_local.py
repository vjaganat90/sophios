...
def run_local(run_args_dict: Dict[str, str], rose_tree: RoseTree,
              use_subprocess: bool, passthrough_args: List[str],
              workflow_name: str, basepath: str = 'autogenerated') -> Optional[int]:
    ...
    stage_input_files(yaml_inputs, yaml_path.parent.absolute(), basepath)
    ...
    cmdline = ' '.join(cmd)
    if run_args_dict.get('generate_run_script', None):
    ...
    if use_subprocess:
        ...
        try:
            if cwl_runner == 'cwltool':
                ...
                if run_args_dict.get('copy_output_files', None) == 'True':
