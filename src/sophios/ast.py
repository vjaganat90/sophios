...
def read_ast_from_disk(homedir: str,
    ...
    Raises:
        ValueError: If the yml file(s) do not exist or the namespace/step configuration is invalid
    ...
    try:
        ...
        # Deliberately broad: any failure while validating (not just
    ...
    for i, step_key in enumerate(steps_keys):
        ...
        if step_key in subkeys:
            ...
            if paths_ns_i == {}:
                raise ValueError(
                    f"Error! namespace {plugin_ns} not found in yaml paths. "
                    "Check 'search_paths_wic' in your config file")
...
def _tree_to_forest(yaml_tree_tuple: YamlTree, tools: Tools) -> YamlForest:
    """Abstracts away the process of traversing an AST, returning all sub-trees encountered as a YamlForest."""
    (step_id, yaml_tree) = yaml_tree_tuple
    ...
    if 'implementations' in wic['wic']:
        ...
        for stepid, back in wic['wic']['implementations'].items():
            implementation_forest = (
                stepid, _tree_to_forest(YamlTree(stepid, back), tools))
    ...
    for i, step_key in enumerate(steps_keys):
        ...
        if step_key in subkeys:
            ...
            sub_yml_forest = _tree_to_forest(
    ...
    for i, step_key in enumerate(steps_keys):
        ...
        if step_key not in subkeys:
            if 'python_script' == step_key:
                ...
                filepath = Path('autogenerated') / f'{unique_id}.cwl'
                ...
                tool_i = Tool(str(filepath), generated_cwl)
