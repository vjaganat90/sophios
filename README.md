...
```
sophios --yaml docs/tutorials/helloworld.wic --graphviz --run_local --copy_output_files --quiet
```
...
## Python API
In addition to the underlying declarative yaml syntax, there is an API for writing Sophios workflows in Python. The Python API is philosophically the exact opposite: users should not have to know any CWL, and in fact all CWL features are hidden unless explicitly exposed.
