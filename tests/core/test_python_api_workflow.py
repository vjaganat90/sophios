...
def _adapter(name: str) -> Path:
    """The path to a packaged CWL adapter, by stem."""
...
def _output_directory_tool(path: Path) -> Path:
    """A built tool whose output is a directory it names itself."""
    path.write_text(
...
def _load_global_config() -> Json:
    """The config CI provisions, read from the user's home."""
...
def _write_manifest(workflow_paths: list[Path]) -> None:
    """Record the generated `.wic` paths for the validation step that follows."""
...
def test_linear_python_workflow_reuses_compiler_edge_inference() -> None:
    ...
    producing step, so inference ran rather than the API pre-resolving it."""
...
def test_step_constructor_accepts_tool_builder_command_line_tool() -> None:
    """A built tool becomes a step, and `step_name` renames both it and its path."""
...
def test_tool_builder_step_bridge_supports_multistep_workflow() -> None:
    """Built tools chain: two steps, ordered, with the workflow output wired through."""
...
def test_output_target_directories_compile_as_runner_local_inputs(tmp_path: Path) -> None:
    ...
    Typed `string` rather than `Directory` because the directory does not exist
    until the step runs; the `glob` refers back to the input that names it."""
...
def test_output_target_directories_reject_absolute_locations(tmp_path: Path) -> None:
    """An output directory is runner-local, so an absolute path is refused."""
    step = Step(clt_path=_output_directory_tool(tmp_path / "write_dir.cwl"))
...
def test_port_namespaces_do_not_accept_string_indexing() -> None:
    """Ports are reached by attribute or by index, never by name as text.

    A string subscript would silently miss a typo; the type rejects it."""
...
def test_compute_request_accepts_compiled_python_workflow() -> None:
    """A compiled workflow becomes a compute request whose JSON round-trips."""
    emit_step = Step(_emit_text_tool(), step_name="emit_text")
...
def test_compute_request_submit_returns_structured_submission(
    ...
    """Submission returns a typed result, not a raw response to be parsed again."""
    posted: dict[str, Any] = {}
    ...
    class FakeResponse:
        ...
        def json(self) -> Json:
            """The decoded body this stub was constructed with."""
            return self._body
    ...
    class FakeSession:
        ...
        def get(self, url: str, *, timeout: tuple[int, int]) -> FakeResponse:
            """Return the canned response for a polled workflow."""
            del timeout
...
def test_compute_request_submit_requires_workflow_id() -> None:
    """A submission without a workflow id is refused before any request is made."""
...
def test_subworkflow_inputs_use_child_workflow_name_and_formal_parameters() -> None:
    ...
    so the two documents agree about the interface between them."""
    touch = Step(clt_path=_adapter("touch"))
    touch.inputs.filename = "empty.txt"
...
def test_step_unknown_attribute_raises_immediately() -> None:
    """Binding an input the tool does not declare fails where it is written."""
    append = Step(clt_path=_adapter("append"))
...
def test_incompatible_step_link_raises_invalid_link_error() -> None:
    """A link between incompatible types is refused at construction."""
    touch = Step(clt_path=_adapter("touch"))
    touch.inputs.filename = "empty.txt"
    append = Step(clt_path=_adapter("append"))
...
def test_explicit_links_must_point_to_prior_steps_in_workflow_list() -> None:
    """A step may only consume what an earlier step produced.

    Order is meaning here: inference scans backwards, so a forward reference
    would describe a workflow that cannot run."""
    touch = Step(clt_path=_adapter("touch"))
    touch.inputs.filename = "empty.txt"
...
def test_explicit_links_must_point_to_workflow_children() -> None:
    """A link may not reach a step outside the workflow that owns it."""
...
def test_workflow_write_wic_exports_source_workflow_with_inferred_edges(tmp_path: Path) -> None:
    """`write_wic` exports the source workflow, with inference still left to the compiler."""
    touch = Step(clt_path=_adapter("touch"))
    touch.inputs.filename = "empty.txt"
...
def test_compiled_artifact_writers_do_not_emit_intermediate_wic_files(
    ...
    """Writing the artifacts leaves no intermediate documents behind."""
...
@pytest.mark.fast
def test_intermediate_source_writer_requires_explicit_flag(tmp_path: Path,
                                                           monkeypatch: pytest.MonkeyPatch) -> None:
    """The compiled source is written out only when asked for."""
    monkeypatch.chdir(tmp_path)
    source = "steps:\n- id: mk_file\n"

    disabled = main_module._write_intermediate_source("wf", source, enabled=False)
    enabled = main_module._write_intermediate_source("wf", source, enabled=True)

    assert disabled is None
    assert enabled == Path("autogenerated") / "wf_source.wic"
    assert list((tmp_path / "autogenerated").glob("*.wic")) == [tmp_path / enabled]
    # Verbatim: the point of writing it is to show what the compiler read.
    assert (tmp_path / enabled).read_text(encoding="utf-8") == source
...
def test_scatter_rejects_unbound_foreign_or_scalar_inputs() -> None:
    ...
    Three refusals, because each would otherwise produce a document that
    compiles and then fails at runtime for a different reason."""
    echo = Step(clt_path=_adapter("echo"))
...
def test_top_level_python_api_exposes_concrete_modules_only() -> None:
    """The package exposes modules, not re-exported names.

    A name re-exported at package level becomes a second import path for one
    thing, and the two drift."""
...
def test_run_local_subprocess_uses_explicit_env_without_global_mutation(
    ...
    """The runner is given its environment; the process environment is untouched.

    Mutating `os.environ` in a library changes behaviour for everything else
    embedding it, and outlives the call."""
    run_args = {
...
def test_run_local_python_api_restores_environment_after_run(monkeypatch: pytest.MonkeyPatch) -> None:
    """Any environment a run needs is restored when the run ends."""
    run_args = {
        "container_engine": "docker",
        "cwl_runner": "cwltool",
    }
...
def test_build_cmd_uses_user_outdir(tmp_path: Path, cwl_runner: str) -> None:
    """A user-supplied `--outdir` is used once, and `--clean` follows the runner."""
...
def test_workflow_run_does_not_forward_python_run_flags_to_runner(
    ...
    """Flags meant for the Python API stay there; only runner flags pass through."""
    touch = Step(clt_path=_adapter("touch"))
    touch.inputs.filename = "empty.txt"
...
def test_workflow_run_writes_virtual_output_directories_without_orphans(
    ...
    """A declared output directory is named in the inputs and not created early."""
    step = Step(clt_path=_output_directory_tool(tmp_path / "write_dir.cwl"))
    step.inputs.outDir = Path("result.outDir")
...
def test_async_serialized_run_uses_normalized_job_inputs(
    ...
    """The async path normalizes inputs the same way the direct path does."""
    workflow: Json = {
