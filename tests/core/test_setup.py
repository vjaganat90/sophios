    ...
    if not schema_store:
        ...
        for workflow_name, workflow_path in (
            ...
            try:
                ...
                if exc.diagnostics and all(item.code is SophiosErrorCode.UNDEFINED_EDGE
                                           for item in exc.diagnostics):
                    # A fragment whose explicit edge comes from an includer has no
                    # standalone schema, exactly as `sophios --generate_schemas`
                    # treats it. Its permissive placeholder remains.
                    continue
                raise
